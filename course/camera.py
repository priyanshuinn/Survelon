...
import time
from datetime import datetime
from course.models import upload_file
# from course.websocket_request import Print


class VideoCamera(object):
    def __init__(self):
        f=upload_file.objects.last()
        url=f.video.url
        extention=url.split('.')[1]
        path= '\\media\\media\\VIDEO'+"."+extention
        with open("course/test.json", 'r') as f:
        ...
        if self.conf["use_dropbox"]:
            ...
            client = dropbox.Dropbox(self.conf["dropbox_access_token"])
        ...
        # self.video = cv2.VideoCapture('G:\\Django Internship\\edu\\course\\sample_data\\cars.mp4') #0
    ...
    def get_frame(self):
        ...
        # with open('G:\\Django Internship\\edu\\course\\config\\test.json', 'r') as f:
        ...
        # vs = cv2.VideoCapture('G:\\Django Internship\\edu\\course\\sample_data\\cars.mp4')
        vs=self.video
        # time.sleep(2.0)
        ...
        if frame is not None:  #************************************************************frame none
            # break    
            # if the log file has not been created or opened
            if self.logFile is None:
                # build the log file path and create/open the log file G:\Django Internship\edu\course\output
                logPath = os.path.join(self.conf["output_path"], self.conf["csv_name"])
                self.logFile = open('log.csv', mode="a")
                ...
                if self.conf["use_dropbox"] and pos == 0:
                ...
                # file then write the column headings
                    ...
                    self.logFile.write("Year,Month,Day,Time (in MPH),Speed\n")
            # resize the frame
            ...
            for (objectID, centroid) in objects.items():
                ...
                # otherwise, if there is a trackable object and it has speed
                # not yet been estimated then estimate it
                    ...
                    if to.lastPoint and not to.estimated:
                        ...
                        self.speed="{:.2f}".format(to.speedMPH)
                        now = datetime.now()
                        self.dt_string = now.strftime("%d/%m/%Y - %H:%M:%S")
                        # Print().Connect(to.speedMPH)
                        # print(to.speedMPH)
                ...
                if not to.logged:
                    ...
                    if to.estimated and to.speedMPH > self.conf["speed_limit"]:
                        ...
                        #     cv2.imwrite(tempFile.path, frame)
                            ...
                            # # ws.send(json.dumps({'value1':to.speedMPH,'value2':time}))
            ...
            if self.conf["display"]:
                ...
                # rect,frame = cv2.imencode('.jpg', frame)[1].tobytes()
                # yield(b'--frame\r\n' + b'Content-Type: image/jpeg\r\n\r\n' + frame + b'\r\n\r\n')
                rect,self.Frame = cv2.imencode('.jpg', frame)
        ...
        # vs.release()
        return self.Frame.tobytes(),self.speed,self.dt_string
